...
if "text" in sys.argv:
    text = True
    from maze.text import maze
elif "turtle" in sys.argv:
    turtle = True
    from maze.turtle import maze
else:
    from maze.text import maze
...
def do_forward(robot_name, steps):
    ...
    """
    if maze.update_position(steps, robot_name):
...
def call_command(command_name, command_arg, robot_name):
    if command_name == 'help':
        ...
        return maze.do_right_turn(robot_name)
    elif command_name == 'left':
        return maze.do_left_turn(robot_name)
...
def robot_start():
    ...
    obs = maze.obstacles.get_obstacles()
    if turtle == False and len(obs) > 0:
        maze.list_obstacles(obs)

    maze.position_x = 0
    maze.position_y = 0
    maze.current_direction_index = 0
